...
const statusConfig = {
    ...
    ongoing: { color: 'text-wood-600 bg-wood-50 border-wood-100', icon: Video, label: 'Ongoing' },
...
export default function SessionCard({ session, onJoin }) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-wood-100 p-5 hover:shadow-md hover:border-wood-200 transition-all duration-300 group">
            ...
            {session.status === 'ongoing' && onJoin && (
                <button
                    onClick={() => onJoin(session)}
                    className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium text-white bg-gradient-to-r from-wood-600 to-wood-500 transition-all duration-300 hover:shadow-lg hover:shadow-wood-300/40"
                >
