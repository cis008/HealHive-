...
import { fetchFlags, fetchAllTherapists, fetchMetrics } from '../../api/mock'
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        Promise.all([fetchFlags(), fetchAllTherapists(), fetchMetrics()])
            .then(([f, t, m]) => { setFlags(f); setTherapists(t); setMetrics(m); setLoading(false) })
    }, [])
                        ...
                        {flags.map(flag => (
                            <div key={flag.id} className={`bg-wood-900/50 border rounded-2xl p-5 ${flag.severity === 'high' ? 'border-red-500/30' : 'border-amber-500/20'
                                ...
                                {flag.status === 'unreviewed' && (
                                    <button className="mt-3 px-4 py-2 rounded-xl text-xs font-medium text-beige-400 border border-beige-500/30 hover:bg-beige-500/10 transition-all">
                                        Mark as Reviewed
                                    </button>
                                )}
                            </div>
                    ...
                    <div className="space-y-3">
                        {therapists.map(t => (
                            <div key={t.id} className="bg-wood-900/50 border border-wood-800/50 rounded-2xl p-5">
                                ...
                                <p className="text-xs text-wood-400 mb-3">{t.bio}</p>
                                {!t.verified && (
                                    <div className="flex gap-2">
                                        <button className="px-4 py-2 rounded-xl text-xs font-medium text-white bg-emerald-500/20 border border-emerald-500/30 hover:bg-emerald-500/30 transition-all flex items-center gap-1">
                                            <CheckCircle className="w-3 h-3" /> Approve
                                        </button>
                                        <button className="px-4 py-2 rounded-xl text-xs font-medium text-red-400 bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 transition-all flex items-center gap-1">
