...
import { Mic, MicOff, Video, VideoOff, PhoneOff, MessageSquare, Maximize2, User } from 'lucide-react'
        ...
        <div className="min-h-screen bg-wood-950 flex flex-col">
            ...
            <div className="flex-1 relative flex items-center justify-center p-4">
                <div className="w-full max-w-4xl aspect-video bg-wood-900 rounded-3xl border border-wood-800 flex items-center justify-center relative overflow-hidden">
                    <div className="text-center">
                        ...
                        <p className="text-wood-500 text-xs mt-1">Connecting...</p>
                    </div>
                    <button className="absolute top-4 right-4 p-2 rounded-xl bg-wood-800/50 text-wood-400 hover:bg-wood-800 transition-all">
            ...
            <div className="flex items-center justify-center gap-4 py-6 bg-wood-950/90 backdrop-blur-sm border-t border-wood-900">
                <button onClick={() => setMic(!mic)}
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-200 ${mic ? 'bg-wood-800 text-white hover:bg-wood-700' : 'bg-red-500/20 text-red-400 hover:bg-red-500/30'
                        }`}>
                    {mic ? <Mic className="w-5 h-5" /> : <MicOff className="w-5 h-5" />}
                </button>
                <button onClick={() => setCam(!cam)}
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-200 ${cam ? 'bg-wood-800 text-white hover:bg-wood-700' : 'bg-red-500/20 text-red-400 hover:bg-red-500/30'
                        }`}>
                    {cam ? <Video className="w-5 h-5" /> : <VideoOff className="w-5 h-5" />}
                </button>
                <button className="w-14 h-14 rounded-2xl bg-wood-800 text-white hover:bg-wood-700 flex items-center justify-center transition-all">
